...
<html>
    <head>
        ...
        <style>
            ...
            :root{
                <?php if (filter_var($siteConfiguration['arix']['borderInput'] ?? false, FILTER_VALIDATE_BOOLEAN)) {
                    echo '--borderInput: 1px solid;
';  
                }?>
                --radiusBox: {{ $siteConfiguration['arix']['radiusBox'] ?? 8 }}px;
                --radiusInput: {{ $siteConfiguration['arix']['radiusInput'] ?? 6 }}px;
                ...
                --fontFamily: '{{
                    ...
                    ][$siteConfiguration['arix']['font'] ?? 'default'] ?? ''
                }}';
            }

            <?php if (($siteConfiguration['arix']['defaultMode'] ?? 'darkmode') === 'darkmode') {
                echo ':root';
            } else {
                echo '.lightmode';
            }?>{
                --image: {{ !empty($siteConfiguration['arix']['backgroundImage']) ? 'url(' . $siteConfiguration['arix']['backgroundImage'] . ')' : 'none' }};
            ...
            <?php if (($siteConfiguration['arix']['defaultMode'] ?? 'darkmode') !== 'darkmode') {
                ...
                --image: {{ !empty($siteConfiguration['arix']['backgroundImageLight']) ? 'url(' . $siteConfiguration['arix']['backgroundImageLight'] . ')' : 'none' }};
            ...
            <?php if (filter_var($siteConfiguration['arix']['backdrop'] ?? false, FILTER_VALIDATE_BOOLEAN)) {
