...
@section('content')
    ...
    <form action="{{ route('admin.arix.layout') }}" method="POST">
        ...
        <div class="input-field">
            <label for="arix:searchComponent">Search or select bar</label>
            <select name="arix:searchComponent" id="arix:searchComponent">
                <option value="1" @if(old('arix:searchComponent', $searchComponent) == '1') selected @endif>Server select bar</option>
                <option value="2" @if(old('arix:searchComponent', $searchComponent) == '2') selected @endif>Searchbar</option>
            </select>
            <small>Choose between server select bar or global searchbar in top navigation.</small>
        </div>
